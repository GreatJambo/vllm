...
class Worker(WorkerBase):
    ...
    def load_model(self) -> None:
        logger.info("DEBUG: GPUWorker.load_model called")
        eep_scale_up = os.environ.get("VLLM_ELASTIC_EP_SCALE_UP_LAUNCH") == "1"
        with self._maybe_get_memory_pool_context(tag="weights"):
            self.model_runner.load_model(eep_scale_up=eep_scale_up)

        logger.info(
            "DEBUG: GPUWorker.load_model finished model_runner.load_model"
        )

        from lmcache.v1.compute.models.utils import VLLMModelTracker

        logger.info(
            f"DEBUG: Registering model to tracker: {self.model_runner.model}"
        )
        VLLMModelTracker.register_model(
            "vllm-instance", self.model_runner.model
        )
        logger.info("DEBUG: Model registered successfully")
    ...
    def reinitialize_distributed(
        ...
        if new_ep_size > old_ep_size:
            ...
            self._eplb_after_scale_up(
                old_ep_size, new_ep_size, global_expert_load
            )
